...
import BeeItemDrawer from '../../src/beeItemDrawer';
...
function setGlobals() {
  ...
  expect(document.getElementById('svgMaze')).toBeDefined();
  expect(document.getElementsByClassName('pegman-location').length).toEqual(1);
...
function validateImages(setup, defaultFlower) {
  ...
  setup.forEach(function (item, col) {
    ...
    try {
      ...
      if (img) {
        expect(img.getAttribute('xlink:href')).toEqual(skin[imgType]);
        expect(img.getAttribute('visibility')).toEqual('visible');
        expect(parseInt(img.getAttribute('x'))).toEqual(50 * col);
        expect(parseInt(img.getAttribute('width'))).toEqual(50);
      }
