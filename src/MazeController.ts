...
class MazeController {
   ...
   */
  playAudio: (sound: string, options?: PlayAudioOptions) => void = (_: string, __: PlayAudioOptions = {}) => {};
  playAudioOnFailure: () => void = () => {};
  loadAudio: (filenames: string[], name: string) => void = (_: string[], __: string) => {};
  getTestResults: (
    levelComplete: boolean,
    options?: GetTestResultsOptions,
  ) => number = (_: boolean, __?: GetTestResultsOptions) => (0);

  constructor(level: MazeData, skin: Skin, config: Configuration, options: Options = {}) {
    this.stepSpeed = 100;
  ...
  animatedFail(forward: boolean, id?: string) {
    ...
    if (
      ...
      if (squareType !== undefined) {
        ...
        this.playAudio('wall' + (this.subtype.wallMap?.[targetY][targetX] || 0));
      }
