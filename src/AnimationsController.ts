...
class AnimationsController {
  ...
  createAnimations_(pegmanId?: string) {
    ...
    if (this.maze.skin.idlePegmanAnimation) {
      ...
      if ((this.maze.skin.idlePegmanCol || 0) > 1 || (this.maze.skin.idlePegmanRow || 0) > 1) {
    ...
    if (this.maze.skin.hittingWallAnimation) {
      ...
      wallAnimationIcon?.setAttribute('height', this.maze.SQUARE_SIZE.toString());
      wallAnimationIcon?.setAttribute('width', this.maze.SQUARE_SIZE.toString());
  ...
  reset(first: boolean) {
    if (first) {
      ...
      if (!this.maze.subtype.allowMultiplePegmen()) {
        this.displayPegman(this.maze.getPegmanX() || 0, this.maze.getPegmanY() || 0, tiles.directionToFrame(this.maze.getPegmanD() || 0));
      }
  ...
  createPegmanAnimation_(options: AnimationOptions) {
    ...
    if (options.col !== undefined) {
      rect?.setAttribute('x', (options.col * this.maze.SQUARE_SIZE + 1 + this.maze.PEGMAN_X_OFFSET).toString());
    }
    if (options.row !== undefined) {
      rect?.setAttribute('y', getPegmanYForRow(this.maze.skin, options.row, this.maze.SQUARE_SIZE).toString());
    }
    ...
    img?.setAttribute('height', (this.maze.PEGMAN_HEIGHT * (options.numRowPegman || 1)).toString());
    img?.setAttribute('width', (this.maze.PEGMAN_WIDTH * (options.numColPegman || 4)).toString());
    ...
    if (options.col !== undefined && options.direction !== undefined) {
      const x = this.maze.SQUARE_SIZE * options.col -
        options.direction * this.maze.PEGMAN_WIDTH + 1 + this.maze.PEGMAN_X_OFFSET;
      img.setAttribute('x', x.toString());
    }
    if (options.row !== undefined) {
      img.setAttribute('y', getPegmanYForRow(this.maze.skin, options.row, this.maze.SQUARE_SIZE).toString());
    }
  }
  ...
  updatePegmanAnimation_(options: Omit<AnimationOptions, 'pegmanImage'>) {
    const rect = document.getElementById(utils.getPegmanElementId(`${options.type}ClipRect`, options.pegmanId));
    if (!rect) {
      return;
    }
    rect.setAttribute('x', ((options.col || 0) * this.maze.SQUARE_SIZE + 1 + this.maze.PEGMAN_X_OFFSET).toString());
    rect.setAttribute('y', getPegmanYForRow(this.maze.skin, options.row || 0, this.maze.SQUARE_SIZE).toString());
    const img = document.getElementById(utils.getPegmanElementId(options.type, options.pegmanId));
    if (!img) {
      return;
    }
    const x = this.maze.SQUARE_SIZE * (options.col || 0) -
        (options.direction || 0) * this.maze.PEGMAN_WIDTH + 1 + this.maze.PEGMAN_X_OFFSET;
    img.setAttribute('x', x.toString());
    const y = getPegmanYForRow(this.maze.skin, options.row || 0, this.maze.SQUARE_SIZE) - this.getPegmanFrameOffsetY_(options.animationRow);
   ...
   */
  scheduleSheetedMovement_(start: Point, delta: Point, numFrames: number, timePerFrame: number, type: string, direction: number, hidePegman: boolean, pegmanId?: string) {
   ...
   */
  scheduleMove(endX: number, endY: number, timeForAnimation: number, pegmanId?: string) {
    ...
    if (this.maze.skin.movePegmanAnimation) {
      ...
      // Show the animation.
      const pegmanIcon = this.getPegmanIcon(pegmanId);
      const movePegmanIcon = document.getElementById(utils.getPegmanElementId(pegmanElements.MOVE, pegmanId));
   ...
   */
  updateSurroundingTiles_(obstacleY: number, obstacleX: number, callback: (element: Element, cell?: Cell) => void) {
    const tileCoords = [
  ...
  scheduleWallHit(_targetX: number, _targetY: number, deltaX: number, deltaY: number, frame: number, pegmanId?: string) {
      ...
      if (numFrames > 1) {
        ...
        timeoutList.setTimeout(() => {
          wallAnimationIcon?.setAttribute('x',
            ...
            (parseInt(wallAnimationIcon?.getAttribute('width') || '0')) / 2).toString());
          wallAnimationIcon?.setAttribute('y',
            (this.maze.SQUARE_SIZE * (pegmanY + 1 + deltaY * 0.5) -
            (parseInt(wallAnimationIcon?.getAttribute('height') || '0'))).toString());
  ...
  scheduleObstacleHit(targetX: number, targetY: number, deltaX: number, deltaY: number, frame: number, pegmanId?: string) {
    // Play the animation
    ...
    timeoutList.setTimeout(() => {
      this.displayPegman((this.maze.getPegmanX(pegmanId) || 0) + deltaX / 2,
  ...
  scheduleDance(victoryDance: boolean, timeAlloted: number, pegmanId?: string) {
    ...
    const originalFrame = tiles.directionToFrame(this.maze.getPegmanD(pegmanId) || 0);
  ...
  setTileTransparent_() {
    ...
    for (let y = 0; y < (this.maze.map?.ROWS || 0); y++) {
      for (let x = 0; x < (this.maze.map?.COLS || 0); x++) {
        ...
        const tileAnimation = document.getElementById('tileAnimation' + tileId) as SVGAnimationElement | null;
        if (tileElement) {
          tileElement.setAttribute('opacity', '0');
        }
  ...
  setPegmanTransparent_(pegmanId?: string) {
    const pegmanFadeoutAnimation = document.getElementById(utils.getPegmanElementId(pegmanElements.FADEOUT, pegmanId)) as SVGAnimationElement | null;
    const pegmanIcon = this.getPegmanIcon(pegmanId);
    if (pegmanIcon) {
  ...
  displayPegman(x: number, y: number, frame: number, pegmanId?: string) {
    const pegmanIcon = this.getPegmanIcon(pegmanId);
    const clipRect = document.getElementById(utils.getPegmanElementId(pegmanElements.CLIP_RECT, pegmanId)) as SVGRectElement | null;
  ...
  getPegmanIcon(pegmanId?: string): SVGImageElement | undefined {
    return (document.getElementById(utils.getPegmanElementId(pegmanElements.PEGMAN, pegmanId)) || undefined) as SVGImageElement | undefined;
  }
