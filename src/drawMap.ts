 ...
 */
export function getPegmanYForRow(skin: Skin, mazeRow: number, squareSize: number = 50): number {
  return Math.floor(squareSize * (mazeRow + 0.5) - (skin.pegmanHeight || 1) / 2 +
    (skin.pegmanYOffset || 0));
}

export function displayPegman(skin: Skin, pegmanIcon: SVGImageElement, clipRect: SVGRectElement, x: number, y: number, frame: number, squareSize: number = 50) {
  const xOffset = skin.pegmanXOffset || 0;
  pegmanIcon.setAttribute('x',
    (x * squareSize - frame * (skin.pegmanWidth || 1) + 1 + xOffset).toString());
  pegmanIcon.setAttribute('y', (getPegmanYForRow(skin, y, squareSize)).toString());

  clipRect.setAttribute('x', (x * squareSize + 1 + xOffset).toString());
  clipRect.setAttribute('y', pegmanIcon.getAttribute('y') || '0');
}

export function addNewPegman(skin: Skin, pegmanId: string | undefined, x: number, y: number, direction: number, svg: SVGSVGElement, squareSize: number = 50) {
...
export function drawMap<T extends Cell, U extends Drawer<T>>(svg: SVGSVGElement, skin: Skin, subtype: Subtype<T, U>, map: MazeMap<T>, squareSize: number = 50) {
  ...
  if (subtype.finish && skin.goalIdle) {
    ...
    finishMarker.setAttribute('x', (squareSize * (subtype.finish.x + 0.5) -
      parseInt(finishMarker.getAttribute('width') || '0') / 2).toString());
    finishMarker.setAttribute('y', (squareSize * (subtype.finish.y + 0.9) -
      parseInt(finishMarker.getAttribute('height') || '0')).toString());
    finishMarker.setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:href',
      skin.goalIdle);
    ...
    for (y = 0; y < map.ROWS; y++) {
      for (x = 0; x < map.COLS; x++) {
        if (map.getTile(y, x) === SquareType.OBSTACLE) {
          ...
          obsIcon.setAttribute('height', (MARKER_HEIGHT * (skin.obstacleScale || 1)).toString());
          obsIcon.setAttribute('width', (MARKER_WIDTH * (skin.obstacleScale || 1)).toString());
          obsIcon.setAttributeNS(
            'http://www.w3.org/1999/xlink', 'xlink:href', skin.obstacleIdle);
          obsIcon.setAttribute('x',
            (squareSize * (x + 0.5) -
            parseInt(obsIcon.getAttribute('width') || '0') / 2).toString());
          obsIcon.setAttribute('y',
            (squareSize * (y + 0.9) -
            parseInt(obsIcon.getAttribute('height') || '0')).toString());
