...
class Bee extends Gatherer<BeeCell, BeeItemDrawer> {
  ...
  constructor(maze: MazeController, config: SubtypeConfiguration) {
    ...
    this.defaultFlowerColor_ = level.flowerType === 'redWithNectar' ?
      'red' : 'purple';
  ...
  createDrawer(svg: SVGSVGElement) {
    if (this.maze_?.map) {
      this.drawer = new BeeItemDrawer(this.maze_.map as MazeMap<BeeCell>, this.skin_, svg, this);
    }
  }
  ...
  shouldCheckPurple(row: number, col: number) {
    return this.isPurpleFlower(row, col) && !this.getMap()?.getVariableCell(row, col)?.isVariableRange();
  }
   ...
   */
  checkedAllClouded() {
    for (let row = 0; row < (this.getMap()?.currentStaticGrid?.length || 0); row++) {
      for (let col = 0; col < (this.getMap()?.currentStaticGrid?.[row]?.length || 0); col++) {
  ...
  isRedFlower(row: number, col: number) {
    ...
    if (this.getMap()?.currentStaticGrid?.[row]?.[col]?.isRedFlower()) {
      return true;
    } else if (this.getMap()?.currentStaticGrid?.[row]?.[col]?.isPurpleFlower()) {
      return false;
    } else {
  ...
  /** @override */
  getEmptyTile(_x: number, _y: number, adjacentToPath: boolean = false, _innerCorner: boolean = false): string {
  ...
  /** @override */
  drawTile(svg: SVGSVGElement, tileSheetLocation: [number, number], row: number, col: number, tileId: string) {
