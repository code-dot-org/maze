...
class Drawer<T extends Cell> {
  ...
  */
   ...
   */
  updateItemImage(row: number, col: number, _running: boolean = false, squareSize: number = SQUARE_SIZE): SVGImageElement | undefined {
   ...
   */
  drawImage_(prefix: string, row: number, col: number, squareSize: number = SQUARE_SIZE): SVGImageElement | undefined {
    let img = this.svg_.querySelector('#' + Drawer.cellId(prefix, row, col)) as SVGImageElement | undefined;
   ...
   * it already exists
   */
  getOrCreateImage_(prefix: string, row: number, col: number, createClipPath: boolean = true, squareSize: number = SQUARE_SIZE): SVGImageElement | undefined {
    const href = this.getAsset(prefix, row, col);

    const imgId = Drawer.cellId(prefix, row, col);

    // Don't create an image if one with this identifier already exists
    let img = this.svg_.querySelector('#' + imgId) as SVGImageElement | undefined;
    if (img) {
      return img;
    }
  ...
  updateOrCreateText_(
    ...
    const pegmanElement = this.svg_.getElementsByClassName('pegman-location')[0];
    let textElement = this.svg_.querySelector('#' + Drawer.cellId(prefix, row, col)) as SVGTextElement | undefined;
    ...
    if (!textElement) {
      ...
      textElement.setAttribute('x', ((col + 1) * squareSize - hPadding).toString());
      textElement.setAttribute('y', ((row + 1) * squareSize - vPadding).toString());
   ...
   */
  drawTile(svg: SVGSVGElement, tileSheetLocation: [number, number], row: number, col: number, tileId: string | number, tileSheetHref: string) {
