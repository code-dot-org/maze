...
describe("wordsearch: randomLetter", function () {
  ...
  it ("randomLetter with restrictions", function () {
    const allChars = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L",
...
describe("wordsearch: drawMapTiles", function () {
  it ("simple wordsearch", function () {
    ...
    maze.map = MazeMap.parseFromOldValues(config.level.map, config.level.initialDirt, Cell);

    const wordSearch = new WordSearch(maze, config);
    const svgMaze = document.getElementById('svgMaze') as unknown as SVGSVGElement | undefined;
