...
describe("Bee", () => {
  describe("isRedFlower", () => {
     ...
     */
    const validate = (flowerType: string, mapValue: string | number, initialDirtValue: number, expected: boolean, msg: string) => {
      ...
      maze.map = MazeMap.parseFromOldValues(config.level.map, config.level.initialDirt, BeeCell);
  ...
  describe('getting nectar', () => {
    it('builds the map', () => {
      ...
      bee.reset();
      expect((bee.getCell(0, 0) as BeeCell | undefined)?.isFlower()).toEqual(true);
