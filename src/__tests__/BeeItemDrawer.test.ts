...
type SetupValues = [number | string, number, number, boolean, boolean, string, string | null];

function validateImages(setup: SetupValues[], defaultFlower: string) {
  // create a 1 row map with all of our values
  const map = [setup.map(item => { return item[0]; })];
  const initialDirtMap = [setup.map(item => { return item[2]; })];
  ...
  const maze = new MazeController(config.level, config.skin as Skin, config as Configuration, {});
  ...
  const drawer = new BeeItemDrawer(maze.map as MazeMap<BeeCell>, skin, svg, bee);
  ...
  setup.forEach((item, col) => {
    ...
    const img = document.getElementById(BeeItemDrawer.cellId('beeItem', 0, col));
    const counter = document.getElementById(BeeItemDrawer.cellId('counter', 0, col));
    const cloud = document.getElementById(BeeItemDrawer.cellId('cloud', 0, col));

    try {
      expect(img === null).toEqual(imgType === null);

      if (img && imgType) {
        expect(img.getAttribute('xlink:href')).toEqual((skin as unknown as {[key: string]: string})[imgType]);
