...
export type CellConstructor = new (tileType: number, value?: number, range?: number) => Cell;
...
class Cell {
   ...
   */
  clone(): Cell {
    const newCell = (this.constructor as typeof Cell).deserialize(this.serialize());
   ...
   */
  static parseFromOldValues(mapCell: string | number, initialDirtCell: string | number | undefined): Cell {
    mapCell = parseInt(mapCell.toString());
    initialDirtCell = initialDirtCell === undefined ? undefined : parseInt(initialDirtCell.toString());
