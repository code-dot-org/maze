...
type ColorList = [string, string, string, string, string, string, string, string, string];
 ...
 */
function svgElement(tag: string, props: {
  [key: string]: string;
}, parent: SVGElement | null | undefined, id: string) {
 ...
 */
function smallCornerSvg(color: string, grid: SVGElement, id: string, size: number, corner: Corner) {
  let finalId = '';
  let shape = '';
 ...
 * @param corner - The enum stating which corner to draw
 */
function triangleSvg(color: string, grid: SVGElement, id: string, size: number, corner: Corner) {
  let finalId = '';
  let shape = '';
  if (corner === Corner.topLeft) {
...
class NeighborhoodDrawer extends Drawer<NeighborhoodCell> {
  ...
  constructor(map: MazeMap<NeighborhoodCell>, skin: Skin, svg: SVGSVGElement, squareSize: number, neighborhood: Neighborhood) {
    super(map, "", svg);
  ...
  resetTile(row: number, col: number) {
    ...
    const cell = this.neighborhood.getCell(row, col) as NeighborhoodCell | undefined;
    cell?.setColor();
   ...
   */
  updateItemImage(row: number, col: number, _running: boolean = false, _squareSize: number = SQUARE_SIZE): SVGImageElement | undefined {
    ...
    if (cell && (cell.getOriginalValue() || 0) > 0) {
      ...
      const newValue = (cell.getCurrentValue() || 0) > 0 && this.showBuckets ? cell.getCurrentValue() || 0 : "";
