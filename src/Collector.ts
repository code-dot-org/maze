...
class Collector<T extends Cell> extends Subtype<T, CollectorDrawer<T>> {
  ...
  drawCorners(svg: SVGSVGElement, row: number, col: number, tileId: string) {
    ...
    if (!this.isWallOrOutOfBounds_(col, row)) {
      Object.keys(corners).filter((corner: keyof (typeof corners)) => {
        ...
        tileClipRect.setAttribute('x', (col * SQUARE_SIZE + (direction[0] + 1) * SQUARE_SIZE / 4).toString());
        tileClipRect.setAttribute('y', (row * SQUARE_SIZE + (direction[1] + 1) * SQUARE_SIZE / 4).toString());
