...
export type SubtypeConstructor<T extends Cell, U extends Drawer<T>> = new (maze: MazeController, options: SubtypeConfiguration) => Subtype<T, U>;
...
class Subtype<T extends Cell, U extends Drawer<T>> extends EventEmitter {
  ...
  createDrawer(svg: SVGSVGElement) {
    if (this.maze_.map) {
      this.drawer = new DirtDrawer(this.maze_.map, this.skin_.dirt || '', svg) as U;
    }
  }
  ...
  getEmptyTile(x: number, y: number, adjacentToPath: boolean = false, innerCorner: boolean = false): string {
    let tile;
   ...
   */
  drawTile(svg: SVGSVGElement, tileSheetLocation: [number, number], row: number, col: number, tileId: string | number) {
