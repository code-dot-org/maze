...
class HarvesterDrawer extends Drawer<HarvesterCell> {
  ...
  constructor(map: MazeMap<HarvesterCell>, skin: Skin, svg: SVGSVGElement, subtype: Subtype<HarvesterCell, HarvesterDrawer>) {
    super(map, '', svg);
    this.skin_ = skin;
  ...
  getAsset(prefix: string, row: number, col: number): string | undefined {
    switch (prefix) {
      ...
      case 'crop':
        {
          const crop = (this.subtype_.getCell(row, col) as HarvesterCell)?.featureName() || 'unknown';
          return (this.skin_ as unknown as {
            [key: string]: string;
          })[crop];
        }
    }
   ...
   * @override
   */
  updateItemImage(row: number, col: number, running: boolean, _squareSize: number = SQUARE_SIZE): SVGImageElement | undefined {
    ...
    if (running) {
      if ((cell.getCurrentValue() || 0) > 0) {
        this.updateOrCreateText_('counter', row, col, (cell.getCurrentValue() || 0).toString());
      } else {
